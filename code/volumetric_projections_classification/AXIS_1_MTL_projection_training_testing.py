...
plt.imshow(train_images[80, :, :,0], cmap='gray')

best_train_images_list=sorted(glob.glob('/content/drive/MyDrive/Colab Notebooks/data_cp303/best slice/flair/train/*'))
best_train_masks_list=sorted(glob.glob('/content/drive/MyDrive/Colab Notebooks/data_cp303/best slice/mask/train/*'))

best_test_images_list=sorted(glob.glob('/content/drive/MyDrive/Colab Notebooks/data_cp303/best slice/flair/test/*'))
best_test_masks_list=sorted(glob.glob('/content/drive/MyDrive/Colab Notebooks/data_cp303/best slice/mask/test/*'))

best_train_images=[]

for img in best_train_images_list:
    best_train_images.append(np.load(img))
    
best_train_images=np.array(best_train_images)

best_test_images=[]

for img in best_test_images_list:
    best_test_images.append(np.load(img))
    
best_test_images=np.array(best_test_images)
best_train_images = np.expand_dims(best_train_images, axis=3)
best_train_images = normalize(best_train_images, axis=1)
best_test_images = np.expand_dims(best_test_images, axis=3)
best_test_images = normalize(best_test_images, axis=1)

plt.imshow(best_train_images[20, :, :, 0], cmap='gray')
